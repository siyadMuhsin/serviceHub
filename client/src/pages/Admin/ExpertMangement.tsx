...
function ExpertManagement() {
  ...
  useEffect(() => {
    ...
    const fetchExperts = async () => {
      try {
        const response = await get_experts(
          currentPage,
          itemsPerPage,
          filter,
          searchQuary
  ...
  const handleConfirmAction = async () => {
    ...
    try {
      ...
      if (confirmationModal.action === "approve") {
        ...
        confirmationModal.action === "unblock"
      ) {
  ...
  const handleRejectWithReason = async () => {
    ...
    setLoading(true);
    try {
      const response = await expert_change_action(
        currentExpert._id!,
        "rejected",
        rejectionReason
      );
      ...
      <div className="min-h-screen bg-[#171730] text-white transition-all pt-14 ">
        ...
        <div
          className={`p-4 md:p-6 transition-all ${
            isSidebarExpanded ? "ml-0 md:ml-64" : "ml-16 md:ml-20"
          }`}
        >
                      ...
                      {expert.status === "approved" && (
                        <div className="flex gap-2">
                          {expert.isBlocked ? (
                            <button
                              onClick={() =>
                                showConfirmation(expert, "unblock")
                              }
                    ...
                    <tr key={expert._id} className="hover:bg-[#333345]">
                      ...
                      <td className="border border-gray-500 px-4 py-2">
                        <span
                          ...
                          ${expert.status === "pending" && "bg-yellow-500"}
                          ${
                            expert.status === "approved" &&
                            (expert?.subscription?.isActive
                              ? "bg-green-500"
                          ...
                          ${expert.status === "rejected" && "bg-red-500"}
                        `}
                        >
                      ...
                      <td className="border border-gray-500 px-4 py-2 space-x-2">
                        {expert.status === "pending" && (
                          <>
                            <button
                              onClick={() =>
                                showConfirmation(expert, "approve")
                              }
