...
const reviewSubmit = async (
  ...
  try {
    console.log(rating, reviewText, expertId);
    const response = await userAPI.post("/review", {
      rating,
      reviewText,
      expertId,
    });
...
const getReviewsByExpertId = async (
  expertId: string,
  page: number,
  limit: number
) => {
  try {
    const response = await userAPI.get(
      `/reviews/${expertId}/?page=${page - 1}&limit=${limit}`
    );
    return response.data;
  } catch (error) {
    throw new Error(error?.response?.data?.message);
